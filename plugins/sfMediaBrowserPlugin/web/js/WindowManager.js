...
sfMediaBrowserWindowManagerObject.prototype.callback = function(value, leaveWindowOpened) {
  this.getTarget().value = value;
  if (!leaveWindowOpened) this.popup.close();
}
...
function sfMediaBrowserWindowManagerCKEditor() {
	this.popup = null;
	this.callbackFnc = null;
}
sfMediaBrowserWindowManagerCKEditor.prototype.init = function(p, c) {
	this.popup = p;
	this.callbackFnc = c;
}
sfMediaBrowserWindowManagerCKEditor.prototype.callback = function(value) {
	CKEDITOR.tools.callFunction(this.callbackFnc,value)
	this.popup.close();
}
