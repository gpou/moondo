...
$(document).ready(function(){
  $('#sf_media_browser_list .file,#sf_media_browser_list .folder').draggable({revert: true, zIndex: 10,
	  stop: function(event, ui) {
		  $(this).find('.icon a').data('dropping',false);
	  }
  });
  $('#sf_media_browser_list .folder,#sf_media_browser_list .up').droppable({accept: '#sf_media_browser_list .file,#sf_media_browser_list .folder', hoverClass: 'movehere',
    drop: function(event, ui) {
	  ...
	  source.find('.icon a').data('dropping',true);
      var target = $(this);
      ...
      var data = {file: source_value, dir: target_value, is_dir: !source.hasClass('file') };
      var callback = function(r, s){
        var status = alertUser(r,s);
        if(status == 'notice' && typeof(source) != 'undefined')
        {
          source.hide('fast');
		  if (source.hasClass('file')) {
			  var clean_name = source_value.lastIndexOf('/') > 0 ? source_value.substr(source_value.lastIndexOf('/')) : source_value;
			  sfMediaBrowserFilePicker.callback(target_value+clean_name, true);
		  }
        }
      };
...
$(document).ready(function(){
  ...
  $('#sf_media_browser_list label.name').dblclick(function() {
	...
	l.siblings('.icon').children('a').data('editing',true);
    ...
    l.children('#validate').click(function(){
      ...
      var data = {file: file, name: new_name, is_dir: !l.parent().hasClass('file')};
      var callback = function(r,s){
        alertUser(r,s);
        if(typeof(r['name']) != 'undefined') {
          ...
          if(!l.parent().hasClass('file')) document.location.href = document.location.href;
			  ...
			  file = link.attr('href', r['url']);
			  sfMediaBrowserFilePicker.callback(r['url'], true);
		  }
        }
      };
