...
class BasesfMediaBrowserActions extends sfActions
{
  ...
  public function executeList(sfWebRequest $request)
  {
	...
	$this->ckEditorCallback = $request->getParameter('CKEditorFuncNum',false);
  }
  ...
  {
	$d = urldecode($request->getParameter('directory'));
	$dir = new sfMediaBrowserFileObject($d);
	$deleted = false;
	if (!cmsFichiers::directoryIsUsed($d)) {
  ...
  public function executeCreateFile(sfWebRequest $request)
  {
    ...
    if($form->isValid())
    {
      ...
      $destination_dir = realpath($this->web_path.'/'.$upload['directory']);
      // thumbnail
  ...
  public function executeMove(sfWebRequest $request)
  {
	...
	if (!$error && $moved) {
		cmsFichiers::renameFile($request->getParameter('file'), $dir->getUrl().'/'.$file->getName(true), $request->getParameter('is_dir','false')!='false');
	}
  ...
  public function executeRename(sfWebRequest $request)
  {
	...
	if (!$error && $renamed) {
		cmsFichiers::renameFile($file->getUrl(), $file->getUrlDir().'/'.$valid_filename, $request->getParameter('is_dir','false')!='false');
	}
  ...
  {syslog(1, $source_file .' - '. $destination_name .' - '. $destination_dir);
  ...
  {
	$f = urldecode($request->getParameter('file'));
    $file = $this->createFileObject($f);
	if (!cmsFichiers::fileIsUsed($f)) {
		$file->delete();
		$this->getUser()->setFlash('notice', 'file.delete');
	} else {
		$this->getUser()->setFlash('error', 'file.delete');	
	} 
    $this->redirect($request->getReferer());
  }
