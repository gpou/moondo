...
<?php if($sf_user->hasFlash('error')): ?>
  ...
  <?php if($sf_user->getFlash('error') == 'directory.delete'): ?>
    <?php echo __('The directory could not be deleted.') ?><br />
	<?php echo __('Probablement, il y a des fichiers dans ce répértoire qui sont utilisés par des lots ou programmes du catalogue immobilier, ou par des pages des sites.'); ?>
  ...
  <?php elseif($sf_user->getFlash('error') == 'file.delete'): ?>
    <?php echo __('The file could not be deleted.') ?><br />
	<?php echo __('Probablement, ce fichier est utilisé par un lot ou un programme du catalogue immobilier, ou par les pages des sites'); ?>
  <?php endif ?>
  </div>
...
<?php // ckeditor integration ?>
<?php if ($ckEditorCallback): ?>
	window.opener.window_manager.init(window, <?php echo $ckEditorCallback ?>);
<?php endif ?>
</script>