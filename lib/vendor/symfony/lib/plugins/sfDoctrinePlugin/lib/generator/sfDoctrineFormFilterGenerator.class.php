...
class sfDoctrineFormFilterGenerator extends sfDoctrineFormGenerator
{
  ...
  public function getWidgetOptionsForColumn($column)
  {
	...
	$years = "array_combine(range(date('Y') - 50, date('Y')), range(date('Y') - 50, date('Y')))";
	switch ($column->getDoctrineType())
    {
      case 'boolean':
        $options[] = "'choices' => array('' => \$i18n->__('yes or no', array(), 'sf_admin'), 1 => \$i18n->__('yes', array(), 'sf_admin'), 0 => \$i18n->__('no', array(), 'sf_admin'))";
        break;
      case 'date':
      case 'datetime':
      case 'timestamp':
        $options[] = "'from_date' => new sfWidgetFormDate(array('years'=>".$years.", 'format'=>'%day%/%month%/%year%')), 'to_date' => new sfWidgetFormDate(array('years'=>".$years.", 'format'=>'%day%/%month%/%year%')), 'template' => \$i18n->__('from %from_date% to %to_date%', array(), 'sf_admin'), 'filter_template' => '%date_range% %empty_checkbox% %empty_label%', 'empty_label'=> \$i18n->__('is empty', array(), 'sf_admin')";
